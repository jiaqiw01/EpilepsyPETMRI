...
# modified with arbitrary number of contrast
...
Modified for T1 + T2_FLAIR + PET_1p inputs
'''
class GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttention(nn.Module):
    def __init__(self, in_num_ch, out_num_ch, block_size=3, first_num_ch=64, input_size=(256,256), sample_factor=(2,2), output_activation='softplus'):
        super(GANShortGeneratorWithSplitInputChannelAttentionAllAndSpatialAttention, self).__init__()
        # in channel can be 3, 6, 9
        self.in_num_ch = in_num_ch
        if self.in_num_ch == 9: # 3 contrasts, lowdose + t1 + t2f
            self.down_1_1 = nn.Sequential(
                    nn.Conv2d(3, first_num_ch, 4, 2, padding=1),
                    nn.LeakyReLU(0.2, inplace=True)
                )
        ...
        elif self.in_num_ch == 6: # 2 contrasts
            self.down_1_1 = nn.Sequential(
                    nn.Conv2d(3, first_num_ch, 4, 2, padding=1),
                    nn.LeakyReLU(0.2, inplace=True)
                )
            ...
            self.down_1_ca = ChannelAttentionLayer(2*first_num_ch, 4)
            self.down_1_comb = nn.Sequential(
                nn.Conv2d(2*first_num_ch, first_num_ch, 1, 1),
                nn.LeakyReLU(0.2, inplace=True)
            )
            ...
            self.down_1_ca = ChannelAttentionLayer(first_num_ch, 4)
            self.down_1_comb = nn.Sequential(
                nn.Conv2d(first_num_ch, first_num_ch, 1, 1),
                nn.LeakyReLU(0.2, inplace=True)
            )
        self.down_2 = Conv_BN_Act(first_num_ch, 2*first_num_ch)
        self.down_3 = Conv_BN_Act(2*first_num_ch, 4*first_num_ch)
    ...
    def forward(self, x):
        if self.in_num_ch == 3: # single contrast
            down_1_1 = self.down_1_1(x)
            down_1_list = down_1_1
            # down_1_2 = self.down_1_2(x[:,1,...].unsqueeze(1))
            # down_1_3 = self.down_1_3(x[:,2,...].unsqueeze(1))
            # down_1_list = torch.cat([down_1_1, down_1_2, down_1_3], dim=1)
        elif self.in_num_ch == 6:
            down_1_1 = self.down_1_1(x[:,0:3,...])            
            down_1_2 = self.down_1_2(x[:,3:6,...])
            down_1_list = torch.cat([down_1_1, down_1_2], dim=1)
        elif self.in_num_ch == 9:
            down_1_1 = self.down_1_1(x[:,0:3,...])            
            down_1_2 = self.down_1_2(x[:,3:6,...])
            down_1_3 = self.down_1_3(x[:,6:9,...])
            down_1_list = torch.cat([down_1_1, down_1_2, down_1_3], dim=1)
        # else:
...
class GANShortGeneratorWithChannelAttentionAllAndSymmetrySpatialAttention(nn.Module):
    def __init__(self, in_num_ch, out_num_ch, first_num_ch=64, input_size=(256,256), sample_factor=(2,2), output_activation='softplus', is_transformer=False):
