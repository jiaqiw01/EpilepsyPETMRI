...
TransUNet with Attention
'''
class TransUNetMultiClass(nn.Module):
    def __init__(self, in_num_ch, out_num_ch, first_num_ch=64, input_size=(256,256), sample_factor=(2,2), is_symmetry=True, output_activation='softplus', is_transformer=False, num_classes=1, embed_dim=128):
        super(TransUNet, self).__init__()

        if num_classes != 0: # feature-wise linear modulation
            self.class_emb = nn.Embedding(num_classes, embed_dim)
            self.class_proj = nn.Linear(embed_dim, first_num_ch)  # Adjust to U-Net channel size
            self.gamma = nn.Linear(first_num_ch, first_num_ch)
            self.beta = nn.Linear(first_num_ch, first_num_ch)
    ...
    def forward(self, x, label_cond=None):
        if label_cond is not None:
            ...
            class_emb = self.class_emb(label_cond)
            class_proj = self.class_proj(class_emb)
            gamma = self.gamma(class_proj).unsqueeze(-1).unsqueeze(-1)  # Expand spatial dims
            beta = self.beta(class_proj).unsqueeze(-1).unsqueeze(-1)
            x = gamma * x + beta
            x = nn.LeakyReLU()(x)
