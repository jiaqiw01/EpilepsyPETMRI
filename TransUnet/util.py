...
def compute_stat(real_B, fake_B, task='reconstruction'):
    if task == 'reconstruction':
        ...
        try:
            ...
            rmse_pred = skimage.metrics.normalized_root_mse(real_B, fake_B)
        except:
            rmse_pred = float('nan')
        try:
            # psnr_pred = compare_psnr(real_B, fake_B)
            psnr_pred = skimage.metrics.peak_signal_noise_ratio(real_B, fake_B)
...
class ZeroDoseDataset(Dataset):
    def __init__(self, dataset_name, data, subj_list, idx_list, brain_mask, block_size=3, contrast_list=['T1', 'T2_FLAIR'], pred_low=False, aug=True, transform=None, missing=False, dropoff=False, skull_strip=False, save_root=None):
    ...
    def augment(self, inputs, target): # input: (n, 160, 192); target: (1, 160, 192)
        ...
        res_input = np.zeros_like(inputs)
        for i in range(n):
        ...
        # if self.save_transform <= 10:
        #     ncol = (inputs.shape[0]+1)//2 + 1
        #     fig, axs = plt.subplots(2, ncol, figsize=(15, 10), facecolor='w', edgecolor='k')
        #     fig.subplots_adjust(hspace=0.01, wspace=0.1)
        #     axs = axs.ravel()
        #     for i, ax in enumerate(axs):
        #         if i < n:
        #             ax.imshow(res_input[i], cmap='gray')
        #             ax.set_title("Input slice")
        #         elif i == n:
        #             ax.imshow(res_target[0], cmap='gray')
        ...
        #             fig.delaxes(axs[j])
        #     self.save_transform += 1
        #     plt.savefig(f"{self.save_root}/augmented_visualization_{self.save_transform}.png")
        #     print("Saved an augmentation...")
    ...
    def __getitem__(self, idx):
        ...
        for contrast_name in self.contrast_list:
            ...
            if contrast_name == "T1c" or contrast_name == "T1":
                if subj_id+'/'+contrast_name+'_GRE' in self.data.keys() and subj_id+'/'+contrast_name+'_SE' in self.data.keys():
                    ...
                    print(f"T1 not found for subject {subj_id}")
                    exit(0)
                    # imgs.append(np.zeros((self.image_size[0], self.image_size[1], 2*self.block_size+1)))
                    mask.append(0)
            elif contrast_name == 'T2_FLAIR':
                if subj_id+'/T2_FLAIR' in self.data.keys():
                    val = self.data[subj_id+'/'+'T2_FLAIR'][:,:,slice_idx-self.block_size:slice_idx+self.block_size+1]
                    # print(val.shape)
                    imgs.append(val)
                    mask.append(1)
                elif subj_id+'/T2_FLAIR_2D' in self.data.keys():
                    # print(subj_id+'/'+contrast_name)
                    val = self.data[subj_id+'/'+'T2_FLAIR_2D'][:,:,slice_idx-self.block_size:slice_idx+self.block_size+1]
                    imgs.append(val)
                    mask.append(1)
                else:
                    print(f"{contrast_name} not found for subject {subj_id}")
                    exit(0)
                    imgs.append(np.zeros((self.image_size[0], self.image_size[1], 2*self.block_size+1)))
                    mask.append(0)
            elif contrast_name == 'PET_10p':
                assert subj_id in self.test_subj, "subject should be in test subjects"
                val = self.synthetic_pet_10p[subj_id][:, :, slice_idx-self.block_size:slice_idx+self.block_size+1]
                print("Used synthetic PET_10p....")
                imgs.append(val)
                mask.append(1)
            else:
                if subj_id+'/'+contrast_name in self.data.keys():
                    val = self.data[subj_id+'/'+contrast_name][:,:,slice_idx-self.block_size:slice_idx+self.block_size+1]
                    imgs.append(val)
                    mask.append(1)
                else:
                    print(f"{contrast_name} not found in subject {subj_id}")
                    exit(0)
        ...
        if self.pred_low:
            if subj_id+'/PET_1p' in self.data.keys():
                targets = self.data[subj_id+'/PET_1p'][:,:,slice_idx:slice_idx+1]
                # print("Using 1p PET as targets...")
            else:
                raise ValueError('Predicting low dose but dose not found!')
        ...
        inputs = np.transpose(inputs, (2, 0, 1)) # n, 160, 192
        targets = np.transpose(targets, (2, 0, 1)) # 1, 160, 192
        ...
        # print(inputs.shape, targets.shape, mask.shape, mask_img.shape)
        out = {'inputs': inputs, 'targets': targets, 'subj_id': subj_id, 'slice_idx': slice_idx, 'mask': mask, 'mask_img': mask_img}
        return out
...
class ZeroDoseDataAll(object):
    def __init__(self, dataset_name, data_path, data_h5_path, train_txt_path, val_txt_path, test_txt_path,
            postfix='', norm_type='mean', batch_size=16, num_fold=5, fold=0, shuffle=True, num_workers=0, block_size=3, 
            contrast_list=['T1'], pred_low=False, transform=None, aug=False, dropoff=False, skull_strip=False, save_root=None):
        ...
        train_dataset = ZeroDoseDataset(dataset_name, data, subj_list_train, idx_list_train, brain_mask, block_size=block_size, contrast_list=contrast_list, pred_low=pred_low, transform=transform, aug=aug, dropoff=dropoff, skull_strip=skull_strip, save_root=save_root)
        val_dataset = ZeroDoseDataset(dataset_name, data, subj_list_val, idx_list_val, brain_mask, block_size=block_size, contrast_list=contrast_list, pred_low=pred_low, transform=None, aug=False, dropoff=dropoff, skull_strip=skull_strip, save_root=save_root)
        test_dataset = ZeroDoseDataset(dataset_name, data, subj_list_test, idx_list_test, brain_mask, block_size=block_size, contrast_list=contrast_list, pred_low=pred_low, transform=None, aug=False, dropoff=False, skull_strip=skull_strip, save_root=save_root)
