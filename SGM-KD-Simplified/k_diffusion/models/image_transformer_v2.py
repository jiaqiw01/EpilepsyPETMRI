...
class TokenMerge(nn.Module):
    ...
    def forward(self, x):
        # print("Before rearrange, ", x.shape) # 256, 256, 9 -> (32, 8) (32, 8) 9 -> 8, 8, 
        x = rearrange(x, "... (h nh) (w nw) e -> ... h w (nh nw e)", nh=self.h, nw=self.w)
        # print("Inside TokenMerge ", x.shape)
        return self.proj(x)
...
class ImageTransformerDenoiserModelV2(nn.Module):
    def __init__(self, levels, mapping, in_channels, out_channels, patch_size, num_classes=0, mapping_cond_dim=0, unet_cond_dim=0):
        super().__init__()
        self.num_classes = num_classes

        self.patch_in = TokenMerge(in_channels+unet_cond_dim, levels[0].width, patch_size)
    ...
    def forward(self, x, sigma, aug_cond=None, class_cond=None, mapping_cond=None, unet_cond=None):
        # Patching
        if unet_cond is not None:
            x = torch.cat([x, unet_cond], dim=1)

        #print("Before patching...", x.shape)
        x = x.movedim(-3, -1)
        x = self.patch_in(x)
