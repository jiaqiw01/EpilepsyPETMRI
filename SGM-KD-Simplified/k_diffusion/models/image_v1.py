...
class ImageDenoiserModelV1(nn.Module):
    # set unet_cond_dim = num_mri_slice (1)
    def __init__(self, c_in, feats_in, depths, channels, self_attn_depths, cross_attn_depths=None, mapping_cond_dim=0, unet_cond_dim=0, cross_cond_dim=0, dropout_rate=0., patch_size=1, skip_stages=0, has_variance=False, num_classes=0):
        super().__init__()
        self.c_in = c_in
        self.channels = channels
        self.unet_cond_dim = unet_cond_dim
        self.class_emb = nn.Embedding(num_classes, feats_in) if num_classes else None
        ...
        self.proj_in = nn.Conv2d(c_in + unet_cond_dim, channels[max(0, skip_stages - 1)], 1)
        self.proj_out = nn.Conv2d(channels[max(0, skip_stages - 1)], c_in + (1 if self.has_variance else 0), 1)
    ...
    # pass unet_cond (mri image) here
    def forward(self, input, sigma, mapping_cond=None, unet_cond=None, cross_cond=None, cross_cond_padding=None, return_variance=False, class_cond=None):
        c_noise = sigma.log() / 4
        ...
        assert self.class_emb is not None
        # print("Class embedding here: ", class_emb.shape)
        mapping_cond_embed = torch.zeros_like(timestep_embed) if mapping_cond is None else self.mapping_cond(mapping_cond)
        mapping_out = self.mapping(timestep_embed + mapping_cond_embed + class_emb)
        cond = {'cond': mapping_out}
        if unet_cond is not None:
            # print("Concat unet cond with input....")
            # print(input.shape, unet_cond.shape)
