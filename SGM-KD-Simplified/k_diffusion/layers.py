...
class Denoiser(nn.Module):
    ...
    def loss(self, input, noise, sigma, visualize=False, **kwargs):
        ...
        if torch.isnan(model_output).any():
            ...
            print("Target is nan.... should not happen...")
            exit(0)
        f_weight = freq_weight_nd(sq_error.shape[2:], self.scales, dtype=sq_error.dtype, device=sq_error.device)
        l = (sq_error * f_weight).flatten(1).mean(1) * c_weight
        if torch.isnan(l).any():
            print("NaN detected in tensor l??????")
            exit(0)
        elif torch.isnan(sq_error).any():
            print("NaN detected in tensor sq_error??????")
            exit(0)
        return l

    def forward(self, input, sigma, **kwargs):
        c_skip, c_out, c_in = [utils.append_dims(x, input.ndim) for x in self.get_scalings(sigma)]
        noise = self.inner_model(input * c_in, sigma, **kwargs)
        return  noise * c_out + input * c_skip, noise
