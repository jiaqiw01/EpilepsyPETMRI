...
def main():
    ...
    p.add_argument('--contrast', nargs='+', default=['T1'],
                   help='contrast')
    p.add_argument('--distance', type=int, default=1, 
                   help='additional neighbor slices')
    p.add_argument('--block_size', type=int, default=1,
                   help='block size')
    p.add_argument('--output_slice', type=int, default=1,
                   help='predict how many PET slice?')
    ...
    p.add_argument('--demo-every', type=int, default=10000,
    ...
    p.add_argument('--evaluate-every', type=int, default=0,
                   help='evaluate every this many steps')
    p.add_argument('--evaluate-n', type=int, default=0,
    ...
    # ])
    # TODO: your own train_set and val_set :)
    # if dataset_config['type'] == 'petmr':
    #     train_set = K.utils.PETMR("/data/jiaqiw01/PET_MRI/data/all_cases_rawspace_complete_max.h5", 
    #                     f"/data/jiaqiw01/PET_MRI/data/slices/finalized_train_slice_t1_t2f_all.txt", 
    #                     img_size=256,
    ...
    #     val_set = K.utils.PETMR("/data/jiaqiw01/PET_MRI/data/all_cases_rawspace_complete_max.h5", 
    #                     f"/data/jiaqiw01/PET_MRI/data/slices/finalized_val_slice_t1_t2f_all.txt", 
    #                     img_size=256,
    #                     normalization_method="-1-1",
    #                     slice_offset=args.block_size, 
    #                     contrast_list=args.contrast,
    ...
    # elif dataset_config['type'] == 'petmrclassemb':
    #     train_set = K.utils.PETMRClassEmb("/data/jiaqiw01/PET_MRI/data/all_cases_rawspace_complete_max.h5", 
    #                     f"/data/jiaqiw01/PET_MRI/data/slices/debug/T1_T2_FLAIR_PET_rawspace_fdg_amy_large/train.txt", 
    #                     img_size=256,
    #                     normalization_method="-1-1",
    #                     slice_offset=args.block_size, 
    #                     contrast_list=args.contrast,
    ...
    #     val_set = K.utils.PETMRClassEmb("/data/jiaqiw01/PET_MRI/data/all_cases_rawspace_complete_max.h5", 
    #                     f"/data/jiaqiw01/PET_MRI/data/slices/debug/T1_T2_FLAIR_PET_rawspace_fdg_amy_large/val.txt", 
    #                     img_size=256,
    #                     normalization_method="-1-1",
    #                     slice_offset=args.block_size, 
    #                     contrast_list=args.contrast,
    ...
    if accelerator.is_main_process:
        try:
            ...
            pass
    # TODO: you may change this according to your dataset
    # here my dataset return {'pet': xx, 'mri': xx}
    image_key = dataset_config.get('image_key', 'pet')
    cond_key = dataset_config.get("cond_key", 'mri')
    ...
    train_dl = data.DataLoader(train_set, args.batch_size, shuffle=True, drop_last=False,
                               num_workers=0, persistent_workers=False, pin_memory=True)
    val_dl = data.DataLoader(val_set, 1, shuffle=False, drop_last=False,
                            num_workers=0, persistent_workers=False, pin_memory=True)
    ...
    with torch.no_grad(), K.models.flops.flop_counter() as fc:
        ...
        extra_args = {}
        extra_args['unet_cond'] = torch.zeros([1, model_config['unet_cond_dim'], size[0], size[1]], device=device)
    ...
    def demo(val_dl, output_slice=1):
        if accelerator.is_main_process:
            tqdm.write('Sampling...')
        n_per_proc = math.ceil(args.sample_n / accelerator.num_processes)
        for i, batch in enumerate(val_dl):
            if (i+1) % 20 == 0 and i < 100:
                mris = batch['mri'].to(device)
                x = torch.randn([accelerator.num_processes, n_per_proc, output_slice, size[0], size[1]], generator=demo_gen).to(device)
                dist.broadcast(x, 0)
                x = x[accelerator.process_index] * sigma_max
                model_fn, extra_args = model_ema, {}
                extra_args['unet_cond'] = mris.to(device)
                extra_args['class_cond'] = batch['label'].to(device)

                model_fn = make_cfg_model_fn(model_ema)
                sigmas = K.sampling.get_sigmas_karras(50, sigma_min, sigma_max, rho=7., device=device)
                x_0 = K.sampling.sample_heun(model_fn, x, sigmas, extra_args=extra_args, disable=not accelerator.is_main_process)
                ...
                filename = f'./results/{args.name}/demo_{step:08}_batch_{i}.png'
                if accelerator.is_main_process:
    ...
    def save():
        accelerator.wait_for_everyone()
        filename = f'./results/{args.name}/ckpt_{step:08}.pth'
        if accelerator.is_main_process:
        ...
        while True:
            ...
            for i, batch in enumerate(train_dl):
                ...
                with accelerator.accumulate(model):
                    ...
                    # print(f"Pet shape: {reals.shape}, MRI shape {mris.shape}")
                    aug_cond = None
                    class_cond, extra_args = None, {}
                    if num_classes:
                        class_cond = batch[class_key]
                        extra_args['class_cond'] = class_cond
                    # add unet_cond to extra_args
                    extra_args['unet_cond'] = mris
                    # print(mris.shape)
                    ...
                    if torch.isnan(losses).any():
                        print("NaN detected in tensor l??????")
                        return
