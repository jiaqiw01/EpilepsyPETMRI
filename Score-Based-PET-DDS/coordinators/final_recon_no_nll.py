...
# @torch.no_grad()
# def p_sample_sub_batch(gt, guidance_img):
#     sub_batch = 24
...
@hydra.main(config_path='../configs', config_name='final_reconstruction_petmr')
def reconstruction(config : DictConfig) -> None:
	...
	# get the data
	dataset = PETMRTest(data_h5="/data/jiaqiw01/PET_MRI/data/all_cases_rawspace_complete_max_no_reslice.h5", 
					subjects=["25609",
							  "24622",
							  "24670",
							  "24625",
					...
					slice_offset=config.block_size,
	...
	# 	inputs, subj_id, slc = dataset[idx]
	# 	# print(pid)
	# 	if inputs.shape[0] != 7:
	# 		print(subj_id, inputs.shape, slc)
	# 		exit(0)
	# 	batch_dict[subj_id].append(inputs)
	...
	# 		inputs = test_batch
	# 		dest = os.path.join(config.nii_save_path, subj)
	# 		# print(unet_cond.shape)
	# 		sub_batch = config.sampling.batch_size
	# 		full_batch = 89
	# 		num_iter = int(np.ceil(full_batch / sub_batch))
	# 		volume = []
	...
	# 			data = inputs[start_idx:end_idx]
	# 			gt = data[:, [0], ...]
	# 			# print(f"gt shape: {gt.shape}")
	# 			if guided:
	# 				guided_img = data[:, 1:, ...].to(config.device) # 8, 7, 256, 256
	# 				print(f"guided_img shape: {guided_img.shape}")
	# 			nll_partial = None

	# 			logg_kwargs = {'log_dir': "./tb", 
	# 				'num_img_in_log': sub_batch, 'sample_num':idx,
	...
	# 			recon, _ = sampler.sample(bs, logg_kwargs=logg_kwargs, logging=False)
	# 			recon = torch.clamp(recon, min=0, max=1)
	# 			volume.append(recon.squeeze(1)) # 16, 1, 256, 256 -> 16, 256, 256

	# 		volume = torch.concat(volume, dim=0) # 89
	# 		pred = volume.cpu().numpy()[:, 0, :, :] # 89/86?, 256, 256
	# 		pred = np.transpose(pred, (1, 2, 0))
	# 		print("====== Final volume shape =======")
	# 		print(pred.shape)
	# 		target = nib.load(os.path.join("/data/jiaqiw01/preprocessed_cases", subj, "reslice_PET_full.nii"))
	# 		aff = target.affine
