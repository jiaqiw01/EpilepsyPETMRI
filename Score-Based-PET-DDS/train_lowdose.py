...
def coordinator():
	...
	config.contrast = ['T1', 'T2_FLAIR', 'PET_1p']
	config.slice_offset = 1
	config.model.in_channels = (2 * config.slice_offset + 1) * len(config.contrast) + 1

	config.label = '_t1_t2f_1p_no_reslice'
	...
	if config.guided_p_uncond is not None: # TODO: set your log_dir here too
