...
def get_standard_score(config, sde, use_ema, ema_ckpt=None, model_ckpt=None, load_path = None, load_model=True):
    if load_model:
    ...
    if load_model: 
        ...
        if use_ema:
            ...
            ema.load_state_dict(torch.load(os.path.join(load_path, ema_ckpt), map_location=config.device))
            ema.copy_to(score.parameters())
            print(f"Ema loaded from {os.path.join(load_path, ema_ckpt)}")
        else:
            print(config.sampling.model_name)
            score.load_state_dict(torch.load(os.path.join(load_path, model_ckpt)))
            print(f"Score model loaded from {os.path.join(load_path, model_ckpt)}")
    return score

def get_standard_sde(config):
    print("Getting a SDE...")
    print(config.sde.type.lower(), config.sde.beta_min, config.sde.beta_max)
...
def get_standard_sampler(config, score, sde, nll, im_shape, observation=None, multimodal=False, score_2=None,
    ...
    if config.sampling.add_corrector:
        corrector = functools.partial(Langevin_sde_corrector,
            nloglik = nll  )
        sample_kwargs['corrector']['corrector_steps'] = config.corrector_steps
        sample_kwargs['corrector']['penalty'] = float(config.sampling.penalty)
        sample_kwargs['corrector']['guidance_imgs'] = guidance_imgs
        sample_kwargs['corrector']['guidance_strength'] = config.sampling.guidance_strength
    ...
    if multimodal:
        print("Initializing a multimodal Basesampler...")
    sampler = BaseSampler(
        score=score, 
        sde=sde,
        predictor=predictor,         
