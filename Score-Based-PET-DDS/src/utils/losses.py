...
def loss_fn(model, x, sde, num_target_slices=1, eps=1e-5):
    ...
    """
    guided = True
    if guided: # TODO: change the number of target slices here according to your config
        x_mri = x[:, num_target_slices:, :,:]
        x = x[:, 0:num_target_slices, :, :]
