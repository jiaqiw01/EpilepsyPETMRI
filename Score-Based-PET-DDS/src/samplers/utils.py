...
def Euler_Maruyama_sde_predictor(
    ...
    if guidance_imgs is not None or guidance_strength is not None:
        ...
        # print("Inside EM predictor....")
        # print(x.shape)
        # print(guidance_imgs.shape)
        # guidance_img_zero = torch.zeros_like(guidance_imgs)
        # assert x.shape == guidance_imgs.shape, "x and guidance image need to have the same shape."
        if multimodal:
            # T1 is the first guidance image
            s_c = score(torch.cat([x,guidance_imgs[:, [0], ...]], dim=1), time_step)
        else:
            print(x.shape, guidance_imgs.shape)
            s_c = score(torch.cat([x,guidance_imgs], dim=1), time_step)
        if score_2:
            s_0 = score_2(torch.cat([x,guidance_imgs[:, [1], ...]], dim=1), time_step)
        else:
            s_0 = score(torch.cat([x,guidance_img_zero], dim=1), time_step)
        if not multimodal:
            guidance_strength = 1.0
            # s = (1 + guidance_strength)*s_c - guidance_strength * s_0
            s = guidance_strength * s_c
        else:
...
def Langevin_sde_corrector(
    ...
    for _ in range(corrector_steps):
        # print("Inside Langevin sde corrector")
        # x.requires_grad_()
        if guidance_imgs is not None or guidance_strength is not None:
            # print("Using guidance image!")
            # zeros = torch.zeros_like(x)
            # guidance_img_zero = torch.zeros_like(guidance_imgs)
            # # assert x.shape == guidance_imgs.shape, "x and guidance image need to have the same shape."
            # s_c = score(torch.cat([x,guidance_imgs], dim=1), time_step)
            ...
            # s = s.detach()
            guidance_img_zero = torch.zeros_like(guidance_imgs)
            # print("Guidance image zero...")
            # print(guidance_img_zero.shape)
            # assert x.shape == guidance_imgs.shape, "x and guidance image need to have the same shape."
            if multimodal:
                # print("Using multimodal method...")
                # T1 is the first guidance image
                s_c = score(torch.cat([x,guidance_imgs[:, [0], ...]], dim=1), time_step)
                assert score_2 is not None, "If Multimodal sampling please provide second SGM"
                s_0 = score_2(torch.cat([x,guidance_imgs[:, [1], ...]], dim=1), time_step)
            else:
                s_c = score(torch.cat([x,guidance_imgs], dim=1), time_step)
                s_0 = score(torch.cat([x,guidance_img_zero], dim=1), time_step)
        ...
        langevin_step_size = 2 * (noise_norm / overall_grad_norm)**2
