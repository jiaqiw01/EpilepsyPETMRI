        ...
        for i in tqdm(range(self.sample_kwargs['start_time_step'], self.sample_kwargs['num_steps'])):     
            ...
            if logging:
                if (i - self.sample_kwargs['start_time_step']) % logg_kwargs['num_img_in_log'] == 0:
                    try:
                        writer.add_image('recon', torchvision.utils.make_grid(x_mean, normalize=True, scale_each=True), i)
                    except:
                        print("Writer add image failed..")
                writer.add_scalar('PSNR', PSNR(x_mean[0, 0].cpu().numpy(), logg_kwargs['ground_truth'][0, 0].cpu().numpy()), i)
                writer.add_scalar('SSIM', SSIM(x_mean[0, 0].cpu().numpy(), logg_kwargs['ground_truth'][0, 0].cpu().numpy()), i)
        if logging:
            return x_mean, writer
        else:
            return x_mean, ret_init
